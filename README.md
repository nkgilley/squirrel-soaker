...
- **Focus Mode**: Camera Module 3 focus is explicit. The current fixed feeder setup uses manual focus with lens position `15.0`, based on a focus bracket against the active crop. Switch to auto-on-capture only if the camera/feeder distance changes or the scene needs per-shot refocus.
